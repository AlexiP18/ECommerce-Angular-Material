...
export class CartItemsComponent {
  ...
  applyCoupon(){
    this.customerService.applyToken(this.couponForm.get(['code'])!.value).subscribe(res =>{
      this.snackBar.open("Cupón Aplicado Satisfactoriamente", 'Cerrar', {
        duration: 5000
      });
      this.getCart();
    }, error => {
  ...
  increaseQuantity(productId: any) {
    ...
    this.customerService.addPlusOnProduct(productId).subscribe(() => {
      this.snackbar.open('Cantidad de Productos Incrementada', 'Cerrar', { duration: 5000 });
      this.getCart();
    });
  }
  ...
  decreaseQuantity(productId: any) {
    ...
    this.customerService.addMinusOnProduct(productId).subscribe(() => {
      this.snackbar.open('Cantidad de Productos Disminuida', 'Cerrar', { duration: 5000 });
      this.getCart();
    });
  }

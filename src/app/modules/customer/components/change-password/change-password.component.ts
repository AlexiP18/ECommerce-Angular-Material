      ...
      this.authService.updatePassword(this.changePasswordForm.value).subscribe(
        (response) => {
          ...
          if (response.id != null) {
            ...
            this.snackBar.open('Contraseña anterior incorrecta', 'Cerrar', { duration: 5000 });
        ...
        (error) => {
          this.isSpinning = false;
          this.snackBar.open('Fallo. Intentelo de nuevo.', 'Cerrar', { duration: 5000, panelClass: 'error-snackbar' });
        }
      );
    } else {
